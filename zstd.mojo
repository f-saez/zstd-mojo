...
# size_t should be Uint but Uint is really incomplete right now, 
...
alias ZSTD_getFrameContentSize = fn(UnsafePointer[UInt8], size_t) -> Int64  # should be UInt64
...
struct ZSTD:
    ...
    fn __init__(inout self):
        ...
        if not self._good:
            self._min_comp_level = 0
            self._max_comp_level = 0
            self._default_comp_level = 0
    ...
    fn compress(self, dst : List[UInt8], src : List[UInt8], comp_level : Int32) -> Int:
        ...
        if self._good:
            ...
            if comp_level<self.min_comp_level():
                ...
                cl = comp_level    
            result = self._handle.get_function[ZSTD_compress]("ZSTD_compress")(dst.unsafe_ptr(), dst.size, src.unsafe_ptr(), src.size, cl)
        return result
    ...
    fn get_frame_content_size(self, src : List[UInt8]) -> Int64: # should be UInt64
        """
        ...
        if self._good:
            ...
            result = self._handle.get_function[ZSTD_getFrameContentSize]("ZSTD_getFrameContentSize")(src.unsafe_ptr(), 10)
        return result

    fn decompress(self, dst : List[UInt8], src : List[UInt8], src_size : Int) -> Int:
        var result = 0
        if self._good:           
            result = self._handle.get_function[ZSTD_decompress]("ZSTD_decompress")(dst.unsafe_ptr(), dst.size, src.unsafe_ptr(), src_size)
        return result
